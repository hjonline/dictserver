...
func main() {
    addr := pflag.StringP("addr", "a", ":8091", "Address to listen on")
    certFile := pflag.StringP("cert", "c", "server.crt", "Path to the TLS certificate file")
    keyFile := pflag.StringP("key", "k", "server.key", "Path to the TLS private key file")
    ...
    err = http.ListenAndServeTLS(*addr, *certFile, *keyFile, router(dict))
    if err != nil {
